...
class TdxInstitutionCrawlSpider(scrapy.Spider):
    name = "tdx_institution_crawl"
    ...
    def start_requests(self):
        ...
        # POST：{"Params":["0","99","20250630","600663","0","1","40"]}
        #       {"Params":["0","99","20250630","600663","0","2","40"]}
        #       {"Params":["0","99","20250630","600663","0","3","40"]}
        # 所有持股一览
        for item in self.ivperStockNoDect:
            payload = {
                "Params": ["0", "99", item['REPORT_DATE'], item['f12'], "0", '1', "40"]
            }
    ...
    def process_item(self, raw_data, ivper_item):
        ...
        item["INSTITUTION_TYPE"] = raw_data[0]  # 机构属性(如:一般法人/基金/QFII等)
        item["INSTITUTION_COUNT"] = raw_data[2]  # 机构数量
        item["HOLDING_QUANTITY"] = f'{round(str_to_int(raw_data[3]) / 10000, 2)}'  # 持仓数量(万股)
        item["HOLDING_VALUE"] = f'{round(str_to_int(raw_data[4]) / 10000, 2)}'  # 持仓市值(万元)
        item["TOTAL_SHARE_RATIO"] = raw_data[5]  # 占总股本比例(%)
        item["FLOAT_SHARE_RATIO"] = raw_data[6]  # 占已流通A股比例(%)
        # print(f'item: {item}')
        return item