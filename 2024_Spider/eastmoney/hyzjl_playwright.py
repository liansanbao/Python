...
class HyzjlPlaywright(EmBase):
    ...
    def setup_page_listeners(self):
        """设置页面事件监听器"""
        # 监听响应事件
        self.search_page.on('response', self.hyzjl_response)
        # 监听对话框事件
        # self.search_page.on('dialog', self.dialogEvent)
        # 可以继续添加其他事件监听器
        # self.search_page.on('request', self.request_handler)
        # self.search_page.on('console', self.console_handler)

    # 对话框处理
    def dialogEvent(self, dialog):
...
if __name__ == '__main__':
    es = HyzjlPlaywright('https://data.eastmoney.com/bkzj/hy.html', 30)
    if not es.isExec():
        es.exec()
        time.sleep(5)
    sys.exit()